...
export function CommandCenter({ data, onNavigate }: { data: CommandCenterDto; onNavigate: (workspace: PrimaryWorkspace) => void }) {
  ...
  return <div className="workspace-view command-center">
    ...
    <section className="metric-grid">
      ...
      <Metric icon="◇" label="تغطية الأدلة" value={metrics.evidenceCount} detail={`${metrics.openPbc} طلب PBC غير مكتمل`} tone={metrics.evidenceCount && metrics.openPbc === 0 ? 'ok' : 'evidence'}/>
    ...
    <section className="dashboard-grid">
      ...
      <div className="panel-glass trace-summary"><SectionHead title="سلامة المسار" subtitle="من البيانات إلى الاستنتاج"/><div className="trace-kpis"><div><strong>{metrics.evidenceCount}</strong><span>أدلة</span></div><div><strong>{metrics.procedures}</strong><span>إجراءات</span></div><div><strong>{metrics.openFindings}</strong><span>نتائج مفتوحة</span></div></div><p className="muted">استخدم مستكشف الأثر للتحقق من الروابط الفعلية بين الدليل والمخاطر والإجراءات.</p><button className="ghost-action full" onClick={() => onNavigate('more')}>استكشاف شبكة الأدلة</button></div>
    </section>
  </div>;
}
